...
% STATES
% ck: alle States verdoppeln, Namen verändern
ar.model(m).px0 = {};
ar.model(m).x = {};
ar.model(m).xNames = {};
ar.model(m).xUnits = {};
...
% INPUTS
% ck: Inputs können von p abhängen, darum auch alle Inputs verdoppeln, Namen verändern
ar.model(m).u = {};
ar.model(m).uUnits = {};
ar.model(m).fu = {};
...
% REACTIONS (or ODES)
% ck: in den Reaktionen die Namen der Parameter und Zustände ändern, 
...
% ck: alle Derived verdoppeln und ersetzungen durchführen
...
% ck: alle Observablen verkoppeln und abh. von Zuständen ersetzen, ebenso
% die errors für die Observablen übernehmen
