...
function arLoadData_Double(name, m, extension, removeEmptyObs, dpPerShoot)
...
disp('  arLoadData_Double Version 2 started ...');
...
ar.model(m).data(d).map.old.response_parameter = ar.model(m).data(d).response_parameter;
ar.model(m).data(d).map.new1.response_parameter = strcat(ar.model(m).data(d).response_parameter,doubleSuffix1);
ar.model(m).data(d).map.new2.response_parameter = strcat(ar.model(m).data(d).response_parameter,doubleSuffix2);
ar.model(m).data(d).response_parameter = ar.model(m).data(d).map.new1.response_parameter;  % beides läuft noch nicht
...
while(~strcmp(C{1},'OBSERVABLES'))
    qu = ismember(ar.model(m).u, [C{1},doubleSuffix1]); %R2013a compatible
    qu2 = ismember(ar.model(m).u, [C{1},doubleSuffix2]); %R2013a compatible
    if(sum(qu+qu2)~=1)
        error('unknown input %s', cell2mat(C{1}));
    end
    ar.model(m).data(d).fu(qu) = doubleOld2New(C{2},[],1);
    ar.model(m).data(d).fu(qu2) = doubleOld2New(C{2},[],2);
...
ar = doubleUpdateMap(ar,m,'pu',d);

% ar = doubleInputsData(ar,m,d);
...
while(~strcmp(C{1},'ERRORS'))
    try
    qyindex1 = ismember(ar.model(m).data(d).y, [cell2mat(C{1}),doubleSuffix1]);
    qyindex2 = ismember(ar.model(m).data(d).y, [cell2mat(C{1}),doubleSuffix2]);
    catch
        C{1}
    end
    if(sum(qyindex1)==1)
        yindex = find(qyindex1);
        value = doubleOld2New(cell2mat(C{1}),[],1);
    end
    if(sum(qyindex2)==1)
        yindex = find(qyindex2);
        value = doubleOld2New(cell2mat(C{1}),[],2);
    end
    if(sum(qyindex1)==0 && sum(qyindex2)==0)
        yindex = length(ar.model(m).data(d).y) + (1:2);
        value = {doubleOld2New(cell2mat(C{1}),[],1),doubleOld2New(cell2mat(C{1}),[],2)};
        fyvalue = {doubleOld2New(cell2mat(C{7}),[],1),doubleOld2New(cell2mat(C{7}),[],2)};
        yNvalue = {doubleOld2New(cell2mat(C{8}),[],1),doubleOld2New(cell2mat(C{8}),[],2)};
    else
        error('multiple matches for %s', cell2mat(C{1}))
    end
    for i=1:length(value)
        ar.model(m).data(d).y{yindex(i)} = value{i};
        ar.model(m).data(d).yUnits(yindex(i),1) = C{2};
        ar.model(m).data(d).yUnits(yindex(i),2) = C{3};
        ar.model(m).data(d).yUnits(yindex(i),3) = C{4};
        ar.model(m).data(d).normalize(yindex(i)) = C{5};
        ar.model(m).data(d).logfitting(yindex(i)) = C{6};
        ar.model(m).data(d).logplotting(yindex(i)) = C{6};
        ar.model(m).data(d).fy{yindex(i),1} = fyvalue{i};
        if(~isempty(yNvalue{i}))
            ar.model(m).data(d).yNames{yindex(i)} = yNvalue{i};
        else
            ar.model(m).data(d).yNames(yindex(i)) = ar.model(m).data(d).y(yindex(i));
        end
        
        if(sum(ismember(ar.model(m).x, ar.model(m).data(d).y{yindex(i)}))>0) %R2013a compatible
            error('%s already defined in STATES', ar.model(m).data(d).y{yindex(i)});
        end
    end
...
% ar.model(m).data(d).py = setdiff(setdiff(vertcat(varlist{:}), union(union(ar.model(m).map.old.x, ar.model(m).map.old.u), ar.model(m).map.old.z)), {ar.model(m).t, ''}); %R2013a compatible
ar.model(m).data(d).py = setdiff(vertcat(varlist{:}), union(ar.model(m).map.new1.x, ar.model(m).map.new2.x)); %R2013a compatible
ar.model(m).data(d).py = setdiff(ar.model(m).data(d).py, union(ar.model(m).map.new1.u, ar.model(m).map.new2.u)); %R2013a compatible
ar.model(m).data(d).py = setdiff(ar.model(m).data(d).py, union(ar.model(m).map.new1.z, ar.model(m).map.new2.z)); %R2013a compatible
ar.model(m).data(d).py = setdiff(ar.model(m).data(d).py, {ar.model(m).t, ''}); %R2013a compatible
ar = doubleUpdateMap(ar,m,'py',d);
...
while(~strcmp(C{1},'INVARIANTS') && ~strcmp(C{1},'DERIVED') && ~strcmp(C{1},'CONDITIONS'))
    ...
    yindex = []; value = cell(0);
    if(sum(qyindex1)==1)
        yindex = [yindex,find(qyindex1)];
        value = [value,{doubleOld2New(cell2mat(C{2}),[],1)}];
    end
    if(sum(qyindex2)==1)
        yindex = [yindex,find(qyindex2)];
        value = [value,{doubleOld2New(cell2mat(C{2}),[],2)}];
    end
    if(sum(qyindex1)==0 && sum(qyindex2)==0)
        error('some observables do not have an error model defined');
%         yindex = length(ar.model(m).data(d).y) + 1;
    elseif sum(qyindex1)>1 || sum(qyindex2)>1
        error('multiple matches for %s', cell2mat(C{1}))
    end
    for i=1:length(yindex)
        ar.model(m).data(d).fystd{yindex(i)} = value{i};
    end
    C = textscan(fid, '%s %q\n',1, 'CommentStyle', ar.config.comment_string);
end
...
ar = doubleUpdateMap(ar,m,'pystd',d);
...
for j=1:length(ar.model(m).data(d).py_sep)
    if isfield(ar.model(m).data(d).py_sep(j),'pars')
        ar.model(m).data(d).py_sep(j).pars = strrep(ar.model(m).data(d).py_sep(j).pars, '_filename', ['_' ar.model(m).data(d).name]);
    end
end
ar = doubleUpdateMap(ar,m,'p',d);
    

% CONDITIONS
C = textscan(fid, '%s %q\n',1, 'CommentStyle', ar.config.comment_string);
ar.model(m).data(d).fp = transpose(ar.model(m).data(d).p);
if size(ar.model(m).data(d).fp,1)==1
    ar.model(m).data(d).fp = ar.model(m).data(d).fp'; % muss Spalte sein
end
if size(ar.model(m).data(d).p,2)==1
    ar.model(m).data(d).p = ar.model(m).data(d).p'; % muss Zeile sein
end
pmodel_tmp = ar.model(m).p;
pdata_tmp = ar.model(m).data(d).p;
% qcondparamodel = ismember(pdata_tmp, strrep(pmodel_tmp, '_filename', ['_' ar.model(m).data(d).name])); %R2013a compatible
% qmodelparacond = ismember(strrep(pmodel_tmp, '_filename', ['_' ar.model(m).data(d).name]), pdata_tmp); %R2013a compatible
% ar.model(m).data(d).fp(qcondparamodel) = strrep(ar.model(m).fp(qmodelparacond), '_filename', ['_' ar.model(m).data(d).name]);

[~,ia,ib] = intersect(pdata_tmp, strrep(pmodel_tmp, '_filename', ['_' ar.model(m).data(d).name])); 
ar.model(m).data(d).fp(ia) = strrep(ar.model(m).fp(ib), '_filename', ['_' ar.model(m).data(d).name]);

while(~isempty(C{1}) && ~strcmp(C{1},'RANDOM'))
    qcondpara = ismember(ar.model(m).data(d).p, [cell2mat(C{1}),doubleSuffix1]); %R2013a compatible
    qcondpara2 = ismember(ar.model(m).data(d).p, [cell2mat(C{1}),doubleSuffix2]); %R2013a compatible
    notfound = true;
    if(sum(qcondpara)>0)
        ar.model(m).data(d).fp{qcondpara} = ['(' doubleOld2New(cell2mat(C{2}),[],1) ')']; %{'px','pu','pv','px0','pcond'}
        notfound = false;
    end
    if(sum(qcondpara)>0)
        ar.model(m).data(d).fp{qcondpara2} = ['(' doubleOld2New(cell2mat(C{2}),[],2) ')'];   
        notfound = false;
    end
    if notfound
...
ar = doubleConditions(ar,m,d);  % doubling data.pcond
...
if(~strcmp(extension,'none') && ( ...
    ...
    doNotReplaceVals = ar.model(m).data(d).map.old.response_parameter;
    doNotReplaceVals = union(doNotReplaceVals,intersect(header,regexprep(ar.model(m).p,[doubleSuffix1,'$'],'')));
    doNotReplaceVals = union(doNotReplaceVals,intersect(header,regexprep(ar.model(m).p,[doubleSuffix2,'$'],'')));
    doNotReplaceVals = union(doNotReplaceVals,intersect(header,regexprep(ar.model(m).data(d).p,[doubleSuffix1,'$'],'')));
    doNotReplaceVals = union(doNotReplaceVals,intersect(header,regexprep(ar.model(m).data(d).p,[doubleSuffix2,'$'],'')));
    [header,data,dataCell] = doubleDataFile(header,data,dataCell,ar.model(m).data(d).prand, doNotReplaceVals);
    ...
    if(sum(qrandis) > 0)
        ...
        for j=1:size(randis,1)
            ...
            if(sum(~isnan(tmpdata(:)))>0 || ~removeEmptyObs)
                ...
                % At this point, replacements of parameter names are done
                % for random effects:
...
% [header,data,dataCell] = doubleDataCondis(header,data,dataCell,pcond);
...
for j=1:length(ar.model(m).data(d).y)
    ...
    if(sum(q)==1)
        ...
        % empirical stds
        yold = regexprep(regexprep(ar.model(m).data(d).y{j},[doubleSuffix1,'$'],''),[doubleSuffix2,'$'],'');
        qstd1 = ismember(header, [yold '_std',doubleSuffix1]); %R2013a compatible
        if(sum(qstd1)==1)
            ar.model(m).data(d).yExpStd(:,j) = data(:,qstd1);
            fprintf(' with stds');
            if(ar.model(m).data(d).normalize(j))
        ...
        qstd2 = ismember(header, [yold '_std',doubleSuffix1]); %R2013a compatible
        if(sum(qstd2)==1)
            ar.model(m).data(d).yExpStd(:,j) = data(:,qstd2);
            fprintf(' with stds');
            if(ar.model(m).data(d).normalize(j))
                ar.model(m).data(d).yExpStd(:,j) = ar.model(m).data(d).yExpStd(:,j) / nfactor(q);
                fprintf(' normalized');
