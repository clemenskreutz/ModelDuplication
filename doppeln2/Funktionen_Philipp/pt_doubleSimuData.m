...
for k = 1:length(ar.model.data.map.old.fy)
    position = find(strcmp(ar.model.data.map.new2.fy(k),ar.model.data.fy));
...
if ~(ar.model.data.yExp(:,1:length(ar.model.data.map.old.fy)) == ...
        ar.model.data.yExp(:,(length(ar.model.data.map.new2.fy)+1):2*length(ar.model.data.map.new2.fy)))
